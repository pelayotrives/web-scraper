...
for block in event_blocks:
    # 5.- We request the name of the event. It is in a particular HTML tag with a unique class.
    event_name = block.find('h2', class_='Typography_body-lg__4bejd')
    if event_name:
        event_name = event_name.text.strip()

    # 6.- We request the date of the event. It is in a particular HTML tag with no-unique class, but we tag it as the only element without 2 particular classes.
    event_date = block.select_one('p:not(.eds-text-color--ui-600):not(.eds-text-color--ui-800)')
    if event_date:
        event_date = event_date.text.strip()

    # 7.- We request the location of the event. It is also in a particular HTML tag with a unique class.
